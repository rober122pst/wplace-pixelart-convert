...
// Retorna o objeto completo da paleta que melhor corresponde à cor dada
// O pixelColor é um array [r, g, b]
function findClosestColorObject(pixelColor) {
    let minDiff = Infinity;
    let closestColor = null;

    for (const colorObj of colorPalette) {
        const diff = Math.abs(pixelColor[0] - colorObj.rgb[0]) + Math.abs(pixelColor[1] - colorObj.rgb[1]) + Math.abs(pixelColor[2] - colorObj.rgb[2]);
        if (diff < minDiff) {
            minDiff = diff;
...
// Retorna um array [r, g, b] para ser usado pelas funções de dithering
const fixedRgbPalette = colorPalette.map(c => c.rgb);